...
import { downloadFile, savePdfDoc, isTauri } from "@/lib/tauri-download";
...
export default function Page() {
  ...
  const registerFontFace = (font: CustomFont) => {
    const fontFace = new FontFace(
      font.name,
      `url(data:font/${font.format};base64,${font.base64})`,
      {
        style: "normal",
        weight: "normal",
      },
  ...
  const saveProject = () => {
    try {
      const customPresets = stylePresets.filter(
        (p) => !BUILT_IN_PRESETS.some((b) => b.name === p.name),
      );
      const payload = {
      ...
      const stored = JSON.parse(
        localStorage.getItem("rassam-projects") || "[]",
      );
  ...
  const openProject = (p: any) => {
    ...
    try {
      ...
      if (Array.isArray(d.customPresets))
        setStylePresets([...BUILT_IN_PRESETS, ...d.customPresets]);
      if (d.themeSettings?.mermaidTheme)
        setMermaidTheme(d.themeSettings.mermaidTheme);
    } catch {}
    setPanel("");
  ...
  const exportProjectFile = async () => {
    const data = {
      ...
      nodeStyles,
      linkStyles,
      stylePresets: stylePresets.filter(
        (p) => !BUILT_IN_PRESETS.some((b) => b.name === p.name),
      ),
    };
    const success = await downloadFile({
      data: JSON.stringify(data, null, 2),
      filename: `${projectName}.mermaid-project.json`,
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (d.code) {
          ...
          if (Array.isArray(d.stylePresets))
            setStylePresets([...BUILT_IN_PRESETS, ...d.stylePresets]);
          setPanel("");
          toast.success("پروژه بارگذاری شد");
        }
      } catch {
        toast.error("خطا در خواندن فایل پروژه");
      }
    };
    reader.readAsText(f);
    e.target.value = "";
  ...
  const loadTemplates = () => {
    try {
      const stored = JSON.parse(
        localStorage.getItem("rassam-templates") || "[]",
      );
  ...
  const exportAllTemplates = async () => {
    const allTemplates = [
      ...BUILTIN_TEMPLATES.map((t) => ({ name: t.n, code: t.c })),
      ...customTemplates.map((t: any) => ({ name: t.name, code: t.code })),
    ];
    const success = await downloadFile({
      data: JSON.stringify(allTemplates, null, 2),
      filename: "mermaid-templates.json",
      mimeType: "application/json",
    });
    if (success) {
  ...
  const importTemplates = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        toast.error("خطا در وارد کردن قالب");
      }
    };
    reader.readAsText(f);
    e.target.value = "";
  ...
  const exportPresets = async () => {
    const success = await downloadFile({
      data: JSON.stringify(stylePresets, null, 2),
      filename: "mermaid-presets.json",
      mimeType: "application/json",
    });
    if (success) {
      toast.success("پریست‌ها ذخیره شدند");
    }
  };
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (Array.isArray(d)) {
          ...
          setStylePresets([
            ...stylePresets,
            ...d.filter((p: SP) => !existing.has(p.name)),
          ]);
        }
      } catch {}
    };
  ...
  return (
    <>
        ...
        <div
          ...
          style={{
            animation: splashFade
              ? "splash-fade-out 0.6s ease-out forwards"
              : undefined,
          }}
        >
          ...
          <div
            className="absolute"
            style={{ animation: "splash-ring-spin 3s linear infinite" }}
          >
          ...
          <div
            ...
            style={{
              animation:
                "splash-icon-appear 0.8s cubic-bezier(0.34, 1.56, 0.64, 1) 0.2s both, splash-icon-glow 1.5s ease-out 1s both",
            }}
          >
            <img
              src="/icon.png"
          ...
          >
            رسام <span className="text-xs text-[#3d7a6e]">v{version}</span>
          </h1>{" "}
          {/* Subtitle */}
          <p
          ...
          <div
            ...
            style={{
              animation: "splash-subtitle-appear 0.5s ease-out 1.2s both",
            }}
          >
            {[0, 1, 2].map((i) => (
              <div
      ...
      {/* ──── Main App ──── */}
      <div
        className="h-screen flex flex-col font-[Vazirmatn,sans-serif] bg-[#f8fafc] dark:bg-[#0b0b14]"
        style={{ direction: "rtl" }}
      >
        {/* ──── Header ──── */}
        <header className="bg-white dark:bg-[#101018] border-b border-gray-200 dark:border-[#1e1e2e] px-3 py-1.5 flex items-center justify-between shrink-0">
              ...
              <a
                onClick={async (e) => {
                  ...
                  try {
                    ...
                    await open(
                      "https://github.com/A-talebifard/Rassam-persian-memaid-clone/releases",
                    );
                  } catch {
                    window.open(
                      "https://github.com/A-talebifard/Rassam-persian-memaid-clone/releases",
                ...
                className="mr-1.5 inline-flex cursor-pointer items-center rounded-md bg-blue-100 px-1.5 py-0.5 text-[10px] font-medium text-blue-700 hover:bg-blue-200 dark:bg-blue-900 dark:text-blue-300 dark:hover:bg-blue-800 transition-colors"
              >
                v{version}
              </a>
            </span>
          ...
          <div className="flex items-center gap-1.5">
            ...
            />
            <span
              className={`text-[9px] px-1.5 py-0.5 rounded ${projectId ? "bg-[#2a9d8f] text-white" : "bg-gray-100 text-gray-500"}`}
            >
          ...
          <div className="flex items-center gap-1">
            ...
            <label
              className="h-7 w-7 inline-flex items-center justify-center rounded-md hover:bg-accent hover:text-accent-foreground cursor-pointer"
            ...
            <Button
              size="sm"
              className="h-7 bg-[#2a9d8f] hover:bg-[#21867a] text-white gap-1"
          ...
          <div className="w-[35%] border-l border-gray-200 dark:border-[#1e1e2e] flex flex-col bg-white dark:bg-[#101018]">
            ...
            <div className="px-2 py-1 border-t border-gray-200 dark:border-[#1e1e2e] bg-gray-50 dark:bg-[#0e0e18] flex items-center gap-2 text-[9px] text-gray-500 dark:text-gray-500">
              <select
                ...
                className="text-[9px] border border-gray-200 rounded px-1 py-0.5"
              >
                {fontOptions.map((f) => (
          ...
          <div className="flex-1 flex flex-col bg-white dark:bg-[#08080f]">
            <div className="px-2 py-1 border-b border-gray-200 dark:border-[#1e1e2e] bg-gray-50 dark:bg-[#0e0e18] flex justify-between items-center">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-[9px] text-gray-500 w-8 text-center">
                  {zoom}%
                </span>
                <Button
                  variant="ghost"
            ...
            <div
              className="flex-1 overflow-auto p-4 flex items-center justify-center bg-white dark:bg-[#08080f] relative"
              ...
              {error && (
                <div className="absolute inset-0 flex items-center justify-center bg-white/80 dark:bg-[#08080f]/80 backdrop-blur-sm p-4">
                  <div className="max-w-md w-full bg-red-50 dark:bg-[#1a1018] border border-red-200 dark:border-red-900/50 rounded-xl p-4 shadow-lg">
                    ...
                    <pre
                      className="text-[10px] text-red-700/80 dark:text-red-300/70 leading-relaxed whitespace-pre-wrap break-words"
        ...
        <footer className="bg-white dark:bg-[#101018] border-t border-gray-200 dark:border-[#1e1e2e] px-4 py-2 flex items-center justify-between shrink-0">
          <div className="text-[9px] text-gray-400 dark:text-gray-500 flex gap-2">
            ...
            {(nodeStyles.length > 0 || linkStyles.length > 0) && (
              <span className="text-[#2a9d8f]">
                {nodeStyles.length + linkStyles.length} استایل
              </span>
            )}
          </div>
          <a
            href="https://github.com/A-talebifard"
            target="_blank"
            ...
            onClick={async (e) => {
              ...
              try {
                const { open } = await import("@tauri-apps/plugin-shell");
                await open("https://github.com/A-talebifard");
              } catch {
                window.open("https://github.com/A-talebifard", "_blank");
              }
            }}
            className="flex items-center gap-1.5 text-[11px] font-semibold text-gray-600 dark:text-gray-300 hover:text-[#2a9d8f] transition-colors cursor-pointer"
          >
        ...
        {panel && (
          ...
          >
            <div
              className="w-[380px] bg-white dark:bg-[#101018] h-full overflow-auto p-4"
              style={{ direction: "rtl" }}
            ...
            >
              <div className="flex justify-between items-center mb-3">
                <h3 className="text-sm font-bold flex items-center gap-2">
                  {panel === "styler" && (
                    <>
                      <Palette className="h-4 w-4 text-[#2a9d8f]" /> استایل‌دهی
                    </>
                  )}
                  {panel === "templates" && (
                    <>
                      <LayoutTemplate className="h-4 w-4 text-[#2a9d8f]" />{" "}
                      قالب‌ها
                    </>
                  )}
                  {panel === "projects" && (
                    <>
                      <FolderOpen className="h-4 w-4 text-[#2a9d8f]" /> پروژه‌ها
                    </>
                  )}
                  {panel === "export" && (
                    <>
                      <Download className="h-4 w-4 text-[#2a9d8f]" /> خروجی
                    </>
                  )}
                  {panel === "settings" && (
                    <>
                      <Settings className="h-4 w-4 text-[#2a9d8f]" /> تنظیمات
                    </>
                  )}
                </h3>
                <Button
                ...
                <div className="space-y-3">
                  <div>
                    <h4 className="text-xs font-bold mb-2">
                      گره‌ها ({nodeStyles.length})
                    </h4>
                    <select
                      onChange={(e) => {
                        if (e.target.value) addNodeStyle(e.target.value);
                        e.target.value = "";
                      }}
                    ...
                    >
                      ...
                      {nodeIds
                        .filter((id) => !nodeStyles.some((s) => s.id === id))
                        .map((id) => (
                    ...
                    {nodeStyles.map((s, i) => (
                      <div
                        key={i}
                        className="border border-gray-200 dark:border-[#2a2a3a] rounded-lg p-2 mb-2 bg-gray-50 dark:bg-[#14141f]"
                      >
                        <div className="flex justify-between items-center mb-2">
                          <div className="flex items-center gap-2">
                            <div
                              className="w-7 h-5 rounded text-[6px] flex items-center justify-center"
                              style={{
                                background: s.fill,
                        ...
                        <div className="grid grid-cols-3 gap-2 text-[9px]">
                          <div>
                            <span className="text-gray-500">پرکردن</span>
                            <input
                              type="color"
                              value={s.fill}
                              onChange={(e) => {
                                const u = [...nodeStyles];
                                u[i] = { ...u[i], fill: e.target.value };
                                setNodeStyles(u);
                              }}
                          ...
                          <div>
                            <span className="text-gray-500">حاشیه</span>
                            <input
                              type="color"
                              value={s.stroke}
                              onChange={(e) => {
                                const u = [...nodeStyles];
                                u[i] = { ...u[i], stroke: e.target.value };
                                setNodeStyles(u);
                              }}
                              className="w-full h-6 cursor-pointer"
                            />
                          ...
                          <div>
                            <span className="text-gray-500">متن</span>
                            <input
                              type="color"
                              value={s.color}
                              onChange={(e) => {
                  ...
                  <div>
                    <h4 className="text-xs font-bold mb-2">
                      اتصالات ({linkStyles.length})
                    </h4>
                    <div className="flex items-center gap-2 mb-2">
                      <Button
                        size="sm"
                        className="h-6 text-[10px] bg-[#2a9d8f] hover:bg-[#21867a] text-white gap-1"
                        onClick={() =>
                          setLinkStyles([
                            ...linkStyles,
                            {
                              idx: linkStyles.length,
                              stroke: "#9370DB",
                              sw: "2",
                            },
                          ])
                      ...
                      <span className="text-[9px] text-gray-400">
                        تعداد در کد: {linkCount}
                      </span>
                    </div>
                    {linkStyles.map((s, i) => (
                      <div
                        key={i}
                        className="border border-gray-200 dark:border-[#2a2a3a] rounded-lg p-2 mb-2 bg-gray-50 dark:bg-[#14141f] flex items-center gap-2 text-[9px]"
                      >
                        <span>شماره:</span>
                        <input
                          type="number"
                          ...
                          className="w-7 text-[9px] border border-gray-200 rounded px-0.5"
                          min={0}
                        />
                        <span>رنگ:</span>
                        <input
                        ...
                        <Button
                          ...
                          className="h-5 w-5 text-red-500"
                          onClick={() =>
                            setLinkStyles(linkStyles.filter((_, x) => x !== i))
                          }
                        >
                          <X className="h-3 w-3" />
                        </Button>
                  ...
                  <div>
                    ...
                    <div className="flex gap-1.5 mb-2">
                      ...
                      <Button
                        size="sm"
                        className="h-6 text-[9px] bg-[#2a9d8f] hover:bg-[#21867a] text-white"
                        onClick={savePreset}
                      >
                        ذخیره
                      </Button>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {stylePresets.map((p, pi) => (
                        <button
                          key={pi}
                          onClick={() => {
                            nodeIds.forEach((id) => addNodeStyle(id, p.ns[0]));
                          }}
                        ...
                        >
                          ...
                          <div className="flex gap-1 mt-1">
                            {p.ns.slice(0, 3).map((n, j) => (
                              <div
                                key={j}
                                className="w-5 h-3 rounded-sm text-[4px] flex items-center justify-center"
                                style={{
                                  background: n.fill,
              ...
              {panel === "templates" && (
                <div className="space-y-3">
                  ...
                  <div className="border border-dashed border-[#5bb8ac] rounded-lg p-3 bg-[#e6f5f3]">
                    <h4 className="text-xs font-bold mb-2 text-[#2a9d8f]">
                      ذخیره قالب فعلی
                    </h4>
                    <div className="flex gap-1.5">
                      ...
                      <Button
                        size="sm"
                        className="h-7 text-[10px] bg-[#2a9d8f] hover:bg-[#21867a] text-white gap-1"
                  ...
                  <div>
                    <h4 className="text-xs font-bold mb-2 text-gray-600">
                      قالب‌های پیش‌فرض ({BUILTIN_TEMPLATES.length} نمودار)
                    </h4>
                    {(() => {
                      const cats = Array.from(
                        new Set(BUILTIN_TEMPLATES.map((t) => t.cat)),
                      );
                      ...
                      return cats.map((cat) => (
                        <div key={cat} className="mb-3">
                          ...
                          <div className="grid grid-cols-2 gap-1.5">
                            {BUILTIN_TEMPLATES.filter((t) => t.cat === cat).map(
                              (t, i) => (
                                <button
                                  key={i}
                                  onClick={() => {
                                    setCode(t.c);
                                    setPanel("");
                                    toast.success(`قالب «${t.n}» بارگذاری شد`);
                                  }}
                                ...
                                >
                                  ...
                                  <div
                                    className="text-[7px] text-gray-400 mt-1 truncate"
                                    style={{
                                      direction: "ltr",
                                      textAlign: "left",
                                    }}
                  ...
                  {customTemplates.length > 0 && (
                    <div>
                      <h4 className="text-xs font-bold mb-2 text-[#2a9d8f]">
                        قالب‌های ذخیره شده ({customTemplates.length})
                      </h4>
                      <div className="space-y-2 max-h-96 overflow-y-auto">
                        {customTemplates.map((t: any) => (
                          <div
                            key={t.id}
                            className="border border-gray-200 dark:border-[#2a2a3a] rounded-lg bg-gray-50 dark:bg-[#14141f] overflow-hidden"
                          >
                            {editingTemplateId === t.id ? (
                              <div className="p-2 space-y-2">
                                ...
                                <textarea
                                  ...
                                  onChange={(e) =>
                                    setEditingTemplateCode(e.target.value)
                                  }
                                ...
                                <div className="flex gap-1.5">
                                  <Button
                                    size="sm"
                                    className="h-6 text-[9px] bg-green-600 hover:bg-green-700 text-white gap-1"
                                    onClick={() =>
                                      updateTemplate(
                                        t.id,
                                        editingTemplateName,
                              ...
                              <div className="p-2">
                                  ...
                                  <div className="flex items-center gap-1">
                                    <Button
                                      ...
                                      onClick={() => {
                                        setCode(t.code);
                                        setPanel("");
                                        toast.success("قالب بارگذاری شد");
                                      }}
                                      title="بارگذاری"
                                    >
                                ...
                                <div
                                  className="text-[7px] text-gray-400 mt-1"
                                  style={{
                                    direction: "ltr",
                                    textAlign: "left",
                                  }}
                    ...
                    projects.map((p) => (
                      ...
                      >
                        ...
                        <div className="text-[8px] text-gray-400 mt-1">
                          {p.code?.length || 0} کاراکتر ·{" "}
                          {p.fontFamily || "Vazirmatn"}
                        </div>
                      </div>
                    ))
                  )}
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <h4 className="text-xs font-bold mb-2 flex items-center gap-1.5">
                      {darkMode ? (
                        <Moon className="h-3.5 w-3.5 text-[#2a9d8f]" />
                      ) : (
                        <Sun className="h-3.5 w-3.5 text-[#2a9d8f]" />
                      )}{" "}
                      حالت نمایش
                    </h4>
                    <p className="text-[9px] text-gray-500 mb-2">
                      ظاهر برنامه بین روشن و تاریک تغییر دهید.
                    </p>
                  ...
                  <div>
                    <h4 className="text-xs font-bold mb-2 flex items-center gap-1.5">
                      <AlignRight className="h-3.5 w-3.5 text-[#2a9d8f]" /> جهت
                      متن نمودار
                    </h4>
                    <p className="text-[9px] text-gray-500 mb-2">
                      جهت چینش متن در نمودار پیش‌نمایش را تعیین کنید.
                    </p>
                    <div className="flex gap-2">
                      <Button
                        variant={textDirection === "rtl" ? "default" : "ghost"}
                        size="sm"
                      ...
                      <Button
                        variant={textDirection === "ltr" ? "default" : "ghost"}
                        size="sm"
                  ...
                  <div>
                    <h4 className="text-xs font-bold mb-2 flex items-center gap-1.5">
                      <Type className="h-3.5 w-3.5 text-[#2a9d8f]" /> مدیریت
                      فونت‌ها
                    </h4>
                    <p className="text-[9px] text-gray-500 mb-2">
                      فونت‌های محلی (ttf, woff, woff2) را اضافه کنید تا در
                      نمودار استفاده شوند.
                    </p>
                    ...
                    {customFonts.length > 0 && (
                      <div className="space-y-1.5">
                        {customFonts.map((f, i) => (
                          <div
                            key={i}
                            className="flex items-center justify-between border border-gray-200 dark:border-[#2a2a3a] rounded-lg px-2.5 py-1.5 bg-gray-50 dark:bg-[#14141f]"
                          >
                            <div className="flex items-center gap-2">
                    ...
                    {customFonts.length === 0 && (
                      <div className="text-[9px] text-gray-400 text-center py-3">
...
function ExportPanel({
  ...
  const [format, setFormat] = useState<"png" | "jpg" | "svg" | "pdf" | "tiff">(
    "png",
  );
  ...
  const doExport = async () => {
    ...
    try {
      if (!fullCode.trim()) {
        toast.error("کد نمودار خالی است");
        setBusy(false);
        return;
      }
      ...
      if (format === "svg") {
        ...
        if (success) {
          ...
          toast.error("خطا در ذخیره فایل SVG");
        }
        setBusy(false);
        return;
      }
      ...
      if (format === "pdf") {
        ...
        const success = await savePdfDoc(pdfDoc, `${projectName}.pdf`);
        if (success) {
          toast.success("خروجی PDF تولید شد");
        } else {
          toast.error("خطا در ذخیره فایل PDF");
        }
        setBusy(false);
        return;
      } else if (format === "tiff") {
        ...
        canvas.toBlob(
          async (blob: Blob | null) => {
            if (blob) {
              ...
              if (success) {
                toast.success(`خروجی ${format.toUpperCase()} تولید شد`);
              } else {
                toast.error(`خطا در ذخیره فایل ${format.toUpperCase()}`);
              }
            }
            setBusy(false);
          },
      ...
      toast.error(
        "خطا در تولید خروجی: " + (e instanceof Error ? e.message : String(e)),
      );
    }
    setBusy(false);
  };
  ...
  return (
    <div className="text-[10px] space-y-4">
      {/* Format Selection */}
      <div>
        <label className="text-[9px] text-gray-500 mb-1.5 block">
          فرمت خروجی
        </label>
        <div className="flex gap-1.5">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-7 h-5 cursor-pointer rounded border border-gray-200 dark:border-[#2a2a3a]"
              />
      ...
      {/* Export Button */}
      <Button
        className="w-full bg-[#2a9d8f] hover:bg-[#21867a] text-white gap-2 h-9"
        onClick={doExport}
        disabled={busy}
      >
      ...
      {format !== "svg" && (
        <div className="text-[8px] text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-[#14141f] rounded-lg p-2 leading-relaxed">
          خروجی با حفظ فونت و استایل نمودار و نسبت واقعی آن تولید می‌شود. DPI
          بالاتر = کیفیت بهتر برای چاپ. برای چاپ مقاله‌ای، ۳۰۰ DPI توصیه می‌شود.
        </div>
      )}
      <UpdateChecker />
    </div>
